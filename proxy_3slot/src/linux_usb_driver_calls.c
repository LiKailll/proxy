...
#define MAX_DEVICES             5
    ...
    /* We know in advance that the expected device has the following name */
    ...
    bool devFound = false;
    for (i = 0; ((i < MAX_DEVICES) && (i < iNumDevs)); i++) {
        std::string devNameStr(cBufLD[i]);
        for (const std::string& expectedDevName : expectedDeviceNames) {
            if (devNameStr.find(expectedDevName) != std::string::npos) {
                szDeviceName = cBufLD[i];
                devFound = true;
                break;
            }
        }
    ...
    if (NULL == szDeviceName) {
        printf("Error: expected device not found, \n");
        for (const std::string& expectedDevName : expectedDeviceNames) {
            printf("    %s\n", expectedDevName.c_str());
        }
        return 1;
    }
    ...
    if (Status == FTC_SUCCESS) {
        ...
        if (Status == FTC_SUCCESS) {
            printf("\nInitializing USB JTAG interface ...\n");
            Status = pFT2232cMpsseJtag->JTAG_InitDevice(ftHandle, 0);
            ...
            if (DEBUG_USB_DRVR_FUNCS)
                printf("Setting JTAG clock frequency.  Divisor = %d, TCK ~= %dkHz.\n", g_usb_jtag_clk_divider, (12000000 / ((g_usb_jtag_clk_divider + 1) * 2)) / 1000);
            if (Status == FTC_SUCCESS) {
                Status = pFT2232cMpsseJtag->JTAG_SetClock(ftHandle, g_usb_jtag_clk_divider, &dwClockFrequencyHz);
                printf("  Device name = %s\n  USB_JTAG_CLK_DIVIDER = %d, dwClockFrequencyHz = %.1f MHz\n\n", szDeviceName, (int)g_usb_jtag_clk_divider, dwClockFrequencyHz/1e6);
            }
