...
int retry_do() {
    ...
    //printf("RETRY\n");
    if (retry_no == 0)
        printf("core%d: Communication error. Retrying\n", cpu0_or_cpu1-1);
...
read_tap:
    ...
    if ((retry_no >= NUM_SOFT_RETRIES) && (retry_no < NUM_SOFT_RETRIES + NUM_HARD_RETRIES)) {
        ...
        //usb_dbg_test();
        printf("core%d: Resetting or1k\n", cpu0_or_cpu1-1);
        err = dbg_cpu0_write_ctrl(0, 0x02);      // reset or1k

        printf("core%d: Stalling or1k\n", cpu0_or_cpu1-1);
        err = dbg_cpu0_write_ctrl(0, 0x01);      // stall or1k

        err = dbg_cpu0_read_ctrl(0, &stalled);
        if (!(stalled & 0x1)) {
            printf("core%d: or1k should be stalled\n", cpu0_or_cpu1-1);   // check stall or1k
            FT2232_USB_JTAG_CloseDevice();// Close the device
            exit(1);
        }
...
static int is_p65x_fixed_core0_addr(uint32_t addr) {
    int retval = 0;
...
static int get_actual_DC_WISHBONE_chain(uint32_t addr)
{
    ...
    if (g_cpu_no_adjust != 0 && (!is_p65x_fixed_core0_addr(addr))) {
        // for p65x or 37xD
        chain = DC_WISHBONE + cpu0_or_cpu1 - 1;
    }
    return chain;
}
...
int usb_dbg_cpu0_read(uint32_t adr, uint32_t* data, uint32_t length) {
    ...
    //printf("usb_dbg_cpu0_read(): chain = %d\n", cpu0_or_cpu1+g_cpu_no_adjust); fflush(stdout);
    if ((err = usb_dbg_set_chain(cpu0_or_cpu1+g_cpu_no_adjust))) return err;
...
int usb_dbg_cpu0_write(uint32_t adr, uint32_t* data, uint32_t length) {
    ...
    //printf("usb_dbg_cpu0_write(): chain = %d\n", cpu0_or_cpu1+g_cpu_no_adjust); fflush(stdout);
    int i;for (i = 0;i < (length / 4);i++) { data[i] = ntohl(data[i]); }
    if ((err = usb_dbg_set_chain(cpu0_or_cpu1+g_cpu_no_adjust))) return err;
...
int usb_dbg_cpu0_write_ctrl(uint32_t adr, unsigned char data) {
    ...
    //printf("usb_dbg_cpu0_write_ctrl(): cpu0_or_cpu1 = %d (1-core0, 2-core1)\n", cpu0_or_cpu1);
    if ((err = usb_dbg_set_chain(cpu0_or_cpu1+g_cpu_no_adjust))) return err;
...
int usb_dbg_cpu0_write_ctrl_nocrc(uint32_t adr, unsigned char data) {
    ...
    // no ensuring that or1k is stalled here, becuase we're call this from that function
    //printf("usb_dbg_cpu0_write_ctrl(): cpu0_or_cpu1 = %d (1-core0, 2-core1)\n", cpu0_or_cpu1);
    if ((err = usb_dbg_set_chain_nocrc(cpu0_or_cpu1+g_cpu_no_adjust))) return err;
...
int usb_dbg_cpu_write_ctrl(int coreno, uint32_t reset, uint32_t stall) {
    assert(coreno == 0 || coreno == 1);
    if ((err = usb_dbg_set_chain(coreno+1+g_cpu_no_adjust))) return err;
    if ((err = usb_dbg_ctrl(reset, stall))) return err;
    return DBG_ERR_OK;
}

int usb_dbg_cpu_read_ctrl(int coreno, uint32_t* preset, uint32_t* pstall) {
    assert(coreno == 0 || coreno == 1);
    if ((err = usb_dbg_set_chain(coreno+1+g_cpu_no_adjust))) return err;
    uint32_t r, s;
    if ((err = usb_dbg_ctrl_read(&r, &s))) return err;
...
void set28P65xGslsselToZero() {
    if ((g_chip_type != CHIP_QXS320F28P65x) && (g_chip_type != CHIP_QXS320F2837xD)) return;
