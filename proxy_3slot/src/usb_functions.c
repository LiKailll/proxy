...
void usb_dbg_test() {
    ...
    uint32_t npc_address = 0x007f040c;
    uint32_t ppc_address = 0x007f0410;

    printf("usb_dbg_test(): stalling DSP ... ");
    err = dbg_cpu0_write_ctrl(0, 0x01);
    err = dbg_cpu0_read_ctrl(0, &stalled);
    if (!(stalled & 0x1)) {
        printf("DSP should be stalled, but is not.\n");
        exit(1);
    }
    if (err) {
    ...
    if (!err)
        err = dbg_cpu0_write(21 << 2, &zero, 4);
    if (err) {
    ...
    if (!err) {
        printf("usb_dbg_test(): reading NPC & PPC ...\n");
        err = dbg_cpu0_read(npc_address, &npc, 4);  /* Read NPC */  //modified by wangwk 17-03-14(origin:  (0 << 11) + 16    )
    }
    if (err) {
    ...
    if (!err) {
        err = dbg_cpu0_read(ppc_address, &ppc, 4);  /* Read PPC */ //modified by wangwk 17-03-14(origin:  (0 << 11) + 18    )
    }
    if (err) {
    ...
    printf("usb_dbg_test(): IF3 PC: address = 0x%08x, value = 0x%08x\n", npc_address, npc);
    printf("usb_dbg_test():  ID PC: address = 0x%08x, value = 0x%08x\n", ppc_address, ppc);
...
void usb_write_stream(uint32_t stream, uint32_t num_bits, DWORD dwTapControllerState) {
    ...
    if (DEBUG_USB_DRVR_FUNCS) {
        printf("[DEBUG_USB_DRVR_FUNCS] usb_write_stream: num_bytes = %d, WriteDataBuffer contents for %d bits: 0x", num_bytes, num_bits);
        for (i = 0; i < num_bytes + 1; i++)
...
uint32_t usb_read_stream(uint32_t num_bits, DWORD dwTapControllerState) {
    ...
    if (DEBUG_USB_DRVR_FUNCS)
        printf("[DEBUG_USB_DRVR_FUNCS] usb_read_stream: returned Status: 0x%lx from reading %u bits.\n", Status, num_bits);

    if (Status == FTC_SUCCESS) {

        for (i = 0;i < num_bits;i++) {
            returnVal |= ((ReadDataBuffer[i / 8] >> (i % 8)) & 0x1) << (num_bits - 1 - i);
            //printf("returnVal:%x,i:%d\n",returnVal,i);
        }
    }
...
retry_jtag_init:
    ...
    // Set JTAG TDO to be in output mode to workaround a HW fault in early QXS320F28335
    // On chip reset, TDO pin is in input mode
    //
    if (CHIP_QXS320F2833x == g_chip_type) {
        // reset and stall DSP core
        ...
        // NOTE: must stall DSP core before writing to register, otherwise, no effect
    ...
    // according to RTL design, set GS/LS_SEL to 0, so that JTAG can access chip memory
    //
    if ((CHIP_QXS320F28P65x == g_chip_type)
        || (CHIP_QXS320F2837xS == g_chip_type)
        || (CHIP_QXS320F2837xD == g_chip_type)) {
        usb_set_tap_ir(JI_DEBUG);
        set28P65xGslsselToZero();
    }
...
static void reset_tap(void) {
    ...
    static const uint32_t MAX_FAIL_TIME = 6;
    uint32_t l_fail_times = 0;

    while (id != id_read_at_reset) {
        usb_set_tap_ir(JI_IDCODE);
        id = usb_read_stream(32, RUN_TEST_IDLE_STATE);
        //printf("reset_tap: read ID 0x%08x\n", id);

        ++l_fail_times;
        if (l_fail_times >= MAX_FAIL_TIME) {
...
int retry_do() {
    ...
    //printf("RETRY\n");
    if (retry_no == 0)
        printf("QXC2000DSP (core_index = %d): Communication error. Retrying ...\n", g_dsp_core_index);
...
read_tap:
    ...
    if ((retry_no >= NUM_SOFT_RETRIES) && (retry_no < NUM_SOFT_RETRIES + NUM_HARD_RETRIES)) {
        ...
        //usb_dbg_test();
        printf("Resetting QXC2000DSP (core_index = %d) ... \n", g_dsp_core_index);
        err = dbg_cpu0_write_ctrl(0, 0x02);      // reset DSP

        printf("Stalling QXC2000DSP (core_index = %d) ... \n", g_dsp_core_index);
        err = dbg_cpu0_write_ctrl(0, 0x01);      // stall DSP

        err = dbg_cpu0_read_ctrl(0, &stalled);
        if (!(stalled & 0x1)) {
            printf("QXC2000DSP (core_index = %d) should be stalled, but not.\n", g_dsp_core_index);   // check stall
            FT2232_USB_JTAG_CloseDevice();// Close the device
            exit(1);
        }
...
try_again:
    ...
    current_chain = -1;
    if (DEBUG_CMDS) printf("[DEBUG_CMDS] set_chain: %i\n", chain);
...
/* Sets scan chain without checking CRC */
int usb_dbg_set_chain_nocrc(int chain) {
    uint32_t status, crc_generated, crc_read;
    dbg_chain = chain;
...
try_again:
    ...
    if (DEBUG_GDB_DUMP_DATA && (reg_adr < 0x001f0e00 || reg_adr > 0x001f0fec))
        printf("\n[DEBUG_GDB_DUMP_DATA] send command, type:%d len:%d addr:0x%x (type 6-read 2-write) \n", type, len, adr);
    ...
    /* CRCs must match, otherwise retry */
    //uint32_t tries = 0;
    if ((Status != FTC_SUCCESS) || (crc_read != crc_generated)) {
        retries++;
        if (retries < 2) {
            if (DEBUG_USB_DRVR_FUNCS)
...
/* sends out a command with 32bit address and 16bit length, if len >= 0, without checking CRC */
int usb_dbg_command_nocrc(uint32_t type, uint32_t adr, uint32_t len) {
    uint32_t i, status, crc_generated, crc_read;
    ...
    retries = 0;
try_again:
    usb_dbg_set_chain_nocrc(dbg_chain);
...
try_again:
    usb_dbg_set_chain(dbg_chain);
    if (DEBUG_CMDS) printf("\n[DEBUG_CMDS] usb_dbg_ctrl(reset=%d, stall=%d);\n", reset, stall);
    ...
    WriteDataBuffer[0] |= (mode) << (DBGCHAIN_SIZE + 1);
    WriteDataBuffer[0] |= (reset) << (DBGCHAIN_SIZE + 2);
    // bit 6
    WriteDataBuffer[0] |= (stall) << (DBGCHAIN_SIZE + 3);
...
/* writes a ctrl reg without checking CRC */
int usb_dbg_ctrl_nocrc(uint32_t reset, uint32_t stall) {
    uint32_t i, status, crc_generated, crc_read;
    // JTAG driver things
    FTC_STATUS Status = FTC_SUCCESS;
    WriteDataByteBuffer WriteDataBuffer;
...
try_again:
  ...
  //bits 0-4
    ...
    *reset = (ReadDataBuffer[4] >> 6) & 1;
    //stalled bit should be in ReadDataBuffer[4] as bit 6
    *stall = (ReadDataBuffer[4] >> 7) & 1;
    // Now extract the received CRC
    crc_read = 0;
    ...
    //if (DEBUG_CMDS) printf("reset bit %x stalled bit %x:\n",
      // ((ReadDataBuffer[4] >> 6) & 1), ((ReadDataBuffer[4] >> 7) & 1));
    ...
    crc_r = crc_calc(crc_r, ((ReadDataBuffer[4] >> 7) & 1));
    for (i = 0;i < 49;i++)
...
try_again:
    ...
    if (DEBUG_GDB_DUMP_DATA && (reg_adr < 0x007f0e00 || reg_adr > 0x007f0fec))
        printf("[DEBUG_GDB_DUMP_DATA] go len is %dbytes, read=%d (if 0, writing)\n", len, read);
  ...
  // be the GO command and 0 first bit
    ...
    // printf("dwNumBytesReturned=%d\n",dwNumBytesReturned);
    // for(int i=0;i<dwNumBytesReturned*4;i++)
    //   printf("%.2x",ReadDataBuffer[i]);
    // printf("\n\n");
    if (Status != FTC_SUCCESS)
    ...
    if (read) {
        ...
        // unpack like so
        if (DEBUG_USB_DRVR_FUNCS && (reg_adr < 0x001f0e00 || reg_adr > 0x001f0fec))
            printf("[DEBUG_USB_DRVR_FUNCS] USB read data buffer: 0x");
        for (i = 0;i < len;i++) {
            // get first 3 bits
  ...
  //printf("status:%d\n",status);
    ...
    if ((Status != FTC_SUCCESS) || (crc_read != crc_generated)) {
        retries++;
        if (retries < 8) {
            if (DEBUG_USB_DRVR_FUNCS && (reg_adr < 0x007f0e00 || reg_adr > 0x007f0fec)) printf("usb_functions - usb_dbg_go - CRC fail (%d) try %d. Going again\n", status, retries);
            reset_tap();
            goto try_again;
        } else return DBG_ERR_CRC;
    }
    ...
    if (status != 0) {
        retries++;
        if (retries < 8) {
            if (DEBUG_USB_DRVR_FUNCS && (reg_adr < 0x007f0e00 || reg_adr > 0x007f0fec)) printf("usb_functions - usb_dbg_go - bad status (%d) try %d. Going again\n", status, retries);
            reset_tap();
            goto try_again;
...
/* issues a burst read/write without checking CRC */
int usb_dbg_go_nocrc(unsigned char* data, uint16_t len, uint32_t read) {
    uint32_t status, crc_generated, crc_read;
    int i, j;
    uint8_t data_byte;
    retries = 0;
...
static int is_fixed_jtag_chain0_addr(uint32_t addr) {
    int retval = 0;
    if (   (addr >= 0x00202000 && addr <= 0x00203FFF)   // CPU1 <-> CLA IPC
        || (addr >= 0x00280000 && addr <= 0x00283FFF)   // bootram
        || (addr >= 0x010A0000 && addr <= 0x010AFFFF)   // FLASH CTRL
        || (addr >= 0x20000000 && addr <= 0x206BFFFF)   // EMIF
        || (addr >= 0x80000000 && addr <= 0x9FFFFFFF)   // EMIF
        || (addr >= 0x30000000 && addr <= 0x302013FF)   // FLASH DATA and OTP
        || (addr >= 0x30300000 && addr <= 0x30307FFF)   // ECAT, ESCSS
    ) {
        retval = 1;
    }
    return retval;
...
static int get_actual_DC_WISHBONE_chain(uint32_t addr)
{
    int chain = DC_WISHBONE;
    if (   ((CHIP_QXS320F28P65x == g_chip_type) || (CHIP_QXS320F2837xS == g_chip_type) || (CHIP_QXS320F2837xD == g_chip_type))
        && (!is_fixed_jtag_chain0_addr(addr))) {
        // for p65x, 37xS and 37xD
        chain = DC_WISHBONE + (g_dsp_core_index & 0x1);
    }
    return chain;
}

static int get_actual_core_chain(int coreidx)
{
    assert(coreidx >= 0 && coreidx <= 3);
    int chain = coreidx + g_core_jtag_chain_start;
    if (CHIP_QXS320F28003x == g_chip_type && 2 == coreidx) {
        // 28003x CLA is special
        // here chain must be 2 + 2 = 4, but actual chain defined by HW is 3
        assert(4 == chain);
        --chain;
    }
    return chain;
...
int usb_dbg_wb_read8(uint32_t adr, uint8_t* data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_read8(adr = 0x%08x, &data = %p)\n", adr, data);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_wb_read32(uint32_t adr, uint32_t* data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_read32(adr = 0x%08x, &data = %p)\n", adr, data);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_wb_write8(uint32_t adr, uint8_t data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_write8(adr = 0x%08x, data = %u)\n", adr, data);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_wb_write16(uint32_t adr, uint16_t data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_write16(adr = 0x%08x, data = %u)\n", adr, data);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
    if (err = usb_dbg_command(DBG_WB_WRITE16, adr, 2)) return err;
    if (err = usb_dbg_go((unsigned char*)&data, 2, 0)) return err;
    return DBG_ERR_OK;
}
...
int usb_dbg_wb_write32(uint32_t adr, uint32_t data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_write32(adr = 0x%08x, data = %u)\n", adr, data);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_wb_read_block32(uint32_t adr, uint32_t* data, uint32_t len) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_read_block32(adr = 0x%08x, &data = %p, len = %u)\n", adr, data, len);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_wb_write_block32(uint32_t adr, uint32_t* data, uint32_t len) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_write_block32(adr = 0x%08x, &data = %p, len = %u)\n", adr, data, len);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain(chain)) return err;
...
/* write a block to wishbone without checking CRC */
int usb_dbg_wb_write_block32_nocrc(uint32_t adr, uint32_t* data, uint32_t len) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_wb_write_block32_nocrc(adr = 0x%08x, data = 0x%08x, len = %u)\n", adr, *data, len);
    }
    int chain = get_actual_DC_WISHBONE_chain(adr);
    if (err = usb_dbg_set_chain_nocrc(chain)) return err;
    if (err = usb_dbg_command_nocrc(DBG_WB_WRITE32, adr, len)) return err;
    if (err = usb_dbg_go_nocrc((unsigned char*)data, len, 0)) return err;
    return DBG_ERR_OK;
}
...
int usb_dbg_cpu0_read(uint32_t adr, uint32_t* data, uint32_t length) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_cpu0_read(adr = 0x%08x, &data = %p, length = %u)\n", adr, data, length);
    }
    int chain = get_actual_core_chain(g_dsp_core_index);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_cpu0_write(uint32_t adr, uint32_t* data, uint32_t length) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_cpu0_write(adr = 0x%08x, &data = %p, length = %u)\n", adr, data, length);
    }
    int i;for (i = 0;i < (length / 4);i++) { data[i] = ntohl(data[i]); }
    int chain = get_actual_core_chain(g_dsp_core_index);
    if (err = usb_dbg_set_chain(chain)) return err;
...
int usb_dbg_cpu0_write_ctrl(uint32_t adr, unsigned char data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_cpu0_write_ctrl(adr = 0x%08x, data = 0x%u)\n", adr, data);
    }
...
/* write a cpu module register without checking CRC */
int usb_dbg_cpu0_write_ctrl_nocrc(uint32_t adr, unsigned char data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_cpu0_write_ctrl_nocrc(adr = 0x%08x, data = 0x%u)\n", adr, data);
    }
    // no ensuring that or1k is stalled here, becuase we're call this from that function
    int chain = get_actual_core_chain(g_dsp_core_index);
    if (err = usb_dbg_set_chain_nocrc(chain)) return err;
    if (err = usb_dbg_ctrl_nocrc(data >> 1, data & 0x1)) return err;
    return DBG_ERR_OK;
}
...
int usb_dbg_cpu0_read_ctrl(uint32_t adr, unsigned char* data) {
    if (DEBUG_USB_FUNCTIONS) {
        printf("[usb_funcions.c] usb_dbg_cpu0_read_ctrl(adr = 0x%08x, &data = %p)\n", adr, data);
    }
    // no ensuring that or1k is stalled here, becuase we're call this from that function
    uint32_t r, s;
    int chain = get_actual_core_chain(g_dsp_core_index);
...
int usb_dbg_cpu_write_ctrl(int coreidx, uint32_t reset, uint32_t stall) {
    assert(coreidx >= 0 && coreidx <= 3);
    int chain = get_actual_core_chain(coreidx);
    if (err = usb_dbg_set_chain(chain)) return err;
    if (err = usb_dbg_ctrl(reset, stall)) return err;
    return DBG_ERR_OK;
}

int usb_dbg_cpu_read_ctrl(int coreidx, uint32_t* preset, uint32_t* pstall) {
    assert(coreidx >= 0 && coreidx <= 3);
    int chain = get_actual_core_chain(coreidx);
    if (err = usb_dbg_set_chain(chain)) return err;
    uint32_t r, s;
...
void set28P65xGslsselToZero() {
    if ((g_chip_type != CHIP_QXS320F28P65x)
    ...
    // stall dsp
    //
    usb_dbg_cpu0_write_ctrl(0, 0x01);
    ...
    if (g_show_verbose) {
        ...
        printf("after dbg_cpu0_read_ctrl(), stalled = %d\n", stalled);

        uint32_t valread0, valread1, valread2;
        usb_dbg_wb_read_block32(0x01070120, &valread0, 4);
        usb_dbg_wb_read_block32(0x01070124, &valread1, 4);
        usb_dbg_wb_read_block32(0x01070128, &valread2, 4);
        printf("Before write: Reg[0x01070120] = 0x%08x, Reg[0x01070124] = 0x%08x, Reg[0x01070128] = 0x%08x\n",
                valread0, valread1, valread2);
    }

    uint32_t val2wr = 0x0;
    usb_dbg_wb_write_block32(0x01070120, &val2wr, 4); // DevCfgRegs.FARRAM_SEL0 = 0x0
    usb_dbg_wb_write_block32(0x01070124, &val2wr, 4); // DevCfgRegs.FARRAM_SEL1 = 0x0;
    usb_dbg_wb_write_block32(0x01070128, &val2wr, 4); // DevCfgRegs.CORE_SEL0   = 0x0;
    ...
    if (g_show_verbose) {
        ...
        usb_dbg_wb_read_block32(0x01070124, &valread1, 4);
        usb_dbg_wb_read_block32(0x01070128, &valread2, 4);
        printf("After  write: Reg[0x01070120] = 0x%08x, Reg[0x01070124] = 0x%08x, Reg[0x01070128] = 0x%08x\n",
                valread0, valread1, valread2);
        printf("========== END set28P65xGslsselToZero() ==========\n");
    }
}
